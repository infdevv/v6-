class Tab {
    ...
    navigateTo(url, callback) {
        const termsPasswordMgr = ["login", "sign-in", "signin", "signup"];
        let url_to_test = url.toLowerCase()
        for (let i = 0; i < termsPasswordMgr.length; i++) {
            if (url_to_test.includes(termsPasswordMgr[i])) {
                const userResponse = confirm("Open password manager?");
                
                if (userResponse) {
                    localStorage.setItem("temp_password_mgr_call", "1");
                }
