...
class AboutBrowser {
    constructor(plugins) {
        ...
        this.browserTitle = "Hardened AboutBrowser";
    ...
    async asyncInit() {
        ...
        await new Promise(r=>proxyUsing("https://anything.example.com", "UV", r)); // Can we NOT be calling bro's random ass server?
    ...
    handlePasswordManager() {
        this.openTab(this.resourcesProtocol + "passwordmgr")
    }
    ...
    handleBookmarks() {
        // someone wants to add a bookmark
...
async function init(injectNode) {
    ...
    setInterval(() => {
        if (localStorage.getItem("temp_password_mgr_call") == "1"){
            window.aboutbrowser.handlePasswordManager();
            localStorage.setItem("temp_password_mgr_call","0") // just incase override fucks shit up
        }
    }, 500);
}
